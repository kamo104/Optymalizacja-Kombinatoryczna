...
void Graph::joinGraphs(std::vector<std::vector<int>>& g1,std::vector<std::vector<int>>& g2){ // takes references to graphs as inputs g1 <- g2
...
void Graph::generateRandomGraph(int size, int numOfEdges){
    graph.clear();
    // [size] components
    ...
    int maxEdges = int(size*(size-1)/2);
    ...
    for(int i=0;i<size;i++){
        ...
        for(int &neighbour : components[g1][i]){
            ...
            while(counter<neighbour){
                ...
                possibleEdges.push_back(std::make_tuple(rand()%maxEdges,i,counter));
                counter++;
            }
            counter++;
        }
...
void Graph::readFromStream(std::ifstream &stream){
    int size;
    int n1,n2;
    stream>> size;
...
void Graph::printToStream(std::ofstream &stream){
